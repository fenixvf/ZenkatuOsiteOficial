import { useListBannerObras, useListObrasRecentes, useListTop10Obras, useListEpisodiosRecentes } from "@workspace/api-client-react";
...
function HeroCarousel() {
  ...
  const handleNext = () => setCurrentIndex((prev) => (prev + 1) % bannerObras.length);
  const handlePrev = () => setCurrentIndex((prev) => (prev - 1 + bannerObras.length) % bannerObras.length);
    ...
    <div className="relative w-full aspect-[4/3] md:aspect-[21/9] lg:aspect-[2.5/1] overflow-hidden group">
      <AnimatePresence initial={false} mode="wait">
        ...
        >
          ...
          <img
            src={current.bannerUrl || current.capaUrl || `https://placehold.co/1920x1080/0F1C2E/1E3A8A?text=${current.titulo}`}
            alt={current.titulo}
          ...
          <div className="absolute inset-0 z-20 flex flex-col justify-end p-6 md:p-12 lg:p-20 container max-w-screen-2xl">
            ...
            >
              <div className="flex gap-2 mb-4">
                <Badge variant="secondary" className="bg-primary/20 text-primary-foreground hover:bg-primary/30 border-primary/50">
                  {current.ano}
                </Badge>
              ...
              <div className="flex gap-4">
                ...
                <Button asChild size="lg" variant="secondary" className="font-display tracking-wide px-8 bg-secondary/80 backdrop-blur">
                  <Link href={`/obra/${current.slug}`}>
          ...
          >
            <Link href={`/obra/${obra.slug}`}>
              <div className="group relative aspect-[2/3] rounded-lg overflow-hidden border border-border bg-card transition-all hover:scale-[1.03] hover:border-primary/50 hover:shadow-[0_0_15px_rgba(59,130,246,0.3)]">
                ...
                <Badge className="absolute top-2 right-2 bg-primary text-primary-foreground border-none px-2 py-0.5 text-xs font-bold shadow-md">NOVO</Badge>
                <div className="absolute bottom-0 left-0 right-0 p-3 translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                  <h4 className="font-display font-semibold text-sm md:text-base line-clamp-2">{obra.titulo}</h4>
                  <p className="text-xs text-primary font-medium mt-1 truncate">{obra.generos?.[0]}</p>
                </div>
              </div>
            </Link>
          </motion.div>
          ...
          >
            <Link href={`/obra/${obra.slug}`}>
              <div className="group flex items-center gap-4 bg-card hover:bg-secondary border border-border p-3 rounded-lg transition-colors relative overflow-hidden">
                ...
                <img
                  src={obra.capaUrl || `https://placehold.co/100x150/0F1C2E/1E3A8A?text=Capa`}
                  alt={obra.titulo}
                  className="w-12 h-16 md:w-16 md:h-24 object-cover rounded-md shadow-sm z-10"
                  loading="lazy"
                />
                <div className="flex-1 min-w-0 z-10">
                  <h4 className="font-medium text-sm md:text-base line-clamp-1 group-hover:text-primary transition-colors">{obra.titulo}</h4>
                  <p className="text-xs text-muted-foreground mt-1">{obra.views || 0} visualizações</p>
                </div>
              </div>
            </Link>
          </motion.div>
            ...
            <Link href={`/obra/${ep.obraId}?ep=${ep.id}`}>
              <div className="group relative rounded-lg overflow-hidden border border-border bg-card hover:border-primary/50 transition-all hover:shadow-[0_0_15px_rgba(59,130,246,0.15)]">
                <div className="aspect-video relative">
                  <img
                    src={ep.thumbnailUrl || ep.obraCapaUrl || `https://placehold.co/640x360/0F1C2E/1E3A8A?text=Episodio`}
                    alt={ep.titulo}
                    className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
                    loading="lazy"
                  />
                ...
                <div className="p-3">
                  ...
                  <p className="text-xs text-muted-foreground mt-1 line-clamp-1">{ep.titulo}</p>
                </div>
              </div>
            </Link>
          </motion.div>
