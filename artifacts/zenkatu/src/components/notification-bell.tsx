...
import { Bell, ExternalLink, Megaphone, BellOff, Settings2, Loader2 } from "lucide-react";
...
export function NotificationBell() {
  ...
  const [tab, setTab] = useState<"notifs" | "settings">("notifs");
  ...
  const handleOpen = () => {
    setOpen((v) => !v);
    if (!open) { markAllRead(); setTab("notifs"); }
  };

  const handleNotifClick = (url: string | null | undefined) => {
    setOpen(false);
    if (url && url.startsWith("/")) setLocation(url);
    else if (url) window.open(url, "_blank");
  };

  const handlePrefToggle = async (key: "notifyEpisodios" | "notifyObras") => {
    setSavingPrefs(true);
    const next = { ...push.preferences, [key]: !push.preferences[key] };
    await push.updatePreferences(next).catch(() => {});
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[10px] text-muted-foreground/60 mt-1">
                            {formatDistanceToNow(new Date(n.sentAt), { addSuffix: true, locale: ptBR })}
                          </p>
                        </div>
                        {n.url && (
              ...
              <div className="p-4 space-y-4">
                <div className="space-y-3">
                  ...
                  <div className="space-y-2.5">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <p className="text-sm font-medium text-foreground">Novos episódios</p>
                        <p className="text-xs text-muted-foreground">Quando um ep. novo for lançado</p>
                      </div>
                      <div className="flex items-center gap-2">
                        {savingPrefs && <Loader2 className="w-3 h-3 animate-spin text-muted-foreground" />}
                        <Switch
                          checked={push.preferences.notifyEpisodios}
                          onCheckedChange={() => handlePrefToggle("notifyEpisodios")}
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <p className="text-sm font-medium text-foreground">Novas obras</p>
                        <p className="text-xs text-muted-foreground">Quando um anime novo chegar</p>
                      </div>
                      <Switch
                        checked={push.preferences.notifyObras}
                        onCheckedChange={() => handlePrefToggle("notifyObras")}
                        disabled={savingPrefs}
                      />
                    </div>
                  </div>
                ...
                <div className="pt-3 border-t border-border/60">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-xl text-xs font-medium text-red-400 hover:bg-red-500/10 transition-colors"
                  >
